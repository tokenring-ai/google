...
export default {
  ...
  execute: async ({ agent, positionals }: AgentCommandInputType<typeof inputSchema>): Promise<string> => {
    ...
    const { authorizationUrl, waitForCallback } = googleService.beginAuthorization(accountName, redirectUri);

    agent.chatOutput(
      [`Open this URL to sign in to Google for ${accountName}`, authorizationUrl, "", `TokenRing is listening for the OAuth callback at ${redirectUri}.`].join(
        "\n",
      ),
    );
    ...
    const { isAuthenticated, profile } = await googleService.exchangeAuthorizationCode(accountName, code, redirectUri);
