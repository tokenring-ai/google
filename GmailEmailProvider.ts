...
type GmailHeader = { name?: string | undefined; value?: string | undefined };
type GmailPart = {
  mimeType?: string | undefined;
  filename?: string | undefined;
  headers?: GmailHeader[];
  body?: { data?: string | undefined; size?: number | undefined };
  parts?: GmailPart[];
};
...
export default class GmailEmailProvider implements EmailProvider {
  ...
  async listBoxes(): Promise<EmailBox[]> {
    ...
    return gmailSystemBoxes.filter(box => availableLabelNames.has(box.labelName)).map(({ id, name }) => ({ id, name }));
  }
